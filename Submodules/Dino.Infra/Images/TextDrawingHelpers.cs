...
{
    public static class TextDrawingHelpers
    {
        ...
        public static Bitmap DrawTextToBitmap(string text, Font font, Color color, DrawMethod mode, RectangleF rectanglef,
            Bitmap existingBitmap = null, StringFormat drawFormat = null,
            int? maxFontSize = null)
        {
            drawFormat = drawFormat ?? new StringFormat();
            ...
            switch (mode)
            {
                case DrawMethod.AutosizeAccordingToText:
                    {
                        ...
                        {
                            bmp = existingBitmap ?? new Bitmap(1, 1);
                        }
                        else
                        {
                            bmp = existingBitmap ?? new Bitmap(size.Width, size.Height);
                        }
                ...
                case DrawMethod.AutoWarpInConstantRectangle:
                    {
                        Rectangle rect = Rectangle.Ceiling(rectanglef);
                        bmp = existingBitmap ?? new Bitmap(rect.Width, rect.Height);
            ...
            using (Graphics g = Graphics.FromImage(bmp))
            {
                DrawTextOnGraphic(text, font, color, mode, rectanglef, g, drawFormat, maxFontSize);
        ...
        public static void DrawTextOnGraphic(string text, Font font, Color color, DrawMethod mode, RectangleF rectanglef,
            Graphics g, StringFormat drawFormat = null,
            int? maxFontSize = null)
        {
            drawFormat = drawFormat ?? new StringFormat();
            ...
            switch (mode)
            {
                ...
                case DrawMethod.AutoFitInConstantRectangleWithoutWarp:
                {
                    ...
                    if ((maxFontSize.HasValue) && (fontSize > maxFontSize))
                    {
                    ...
                    g.DrawString(text, new Font(font.FontFamily, fontSize, font.Style, GraphicsUnit.Point),
                        new SolidBrush(color), rectanglef, drawFormat);
                    break;
                }
                case DrawMethod.AutoFitInConstantRectangleWithWarp:
                {
